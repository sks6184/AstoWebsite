...
def _system_instructions(depth_level, answer_style="", answer_language="English"):
    ...
    return (
        ...
        "Write EXACTLY this phrasing: 'In the best window, the Chara Mahadasha activates "
        "[window.jaimini_active_sign] (house [window.jaimini_active_sign_house_from_lagna] from Lagna). "
        ...
        "Taurus for Leo Lagna = house 10 (career), not house 4 (property). The same sign maps to "
        "a completely different house for every Lagna.\n"
        "    ⚠ TOPIC MATCH: For EACH active sign, check whether its house is a primary house for the "
        "question category. If YES → state it directly supports the topic. If NO → name that house's "
        "actual domain (career, income, etc.) and call it indirect/secondary support only. "
        "NEVER invent a property/marriage/career connection from a house that does not govern the topic. "
        "A house 10 Jaimini activation for a property question supports financial authority — "
        "it does NOT directly support home acquisition. Say so explicitly.\n"
        "    Example (property question, Jaimini MD house = 10, AD house = 4): 'In the best window, "
        "the Chara Mahadasha activates Taurus (house 10 from Lagna), which governs career and authority — "
        "indirect support for financial stability. The Chara Antardasha runs in Scorpio (house 4 from Lagna), "
        "the house of home and property — this is a direct property indicator.'\n"
        "    Para 3 — Yogini: Name BOTH the major and sub Yogini. Write EXACTLY: "
        "'In the best window, the Yogini major period is [window.yogini_name] (lord: [window.yogini_alignment.major_lord_name]) "
        "and the Yogini sub-period (antardasha) is [window.sub_yogini_name] (lord: [window.yogini_alignment.sub_lord_name]).' "
        "Then one sentence on how this combination bears on the category. "
        "If sub_yogini_name is null, say 'sub-period data unavailable' — do not omit the sub-period mention.\n"
