...
ANSWER_CACHE_VERSION = "v31-jaimini-both-signs-topic-match"
...
def _system_instructions(depth_level, answer_style="", answer_language="English"):
    ...
    return (
        ...
        "  ### Why We Advise That — REQUIRED: write EXACTLY 5 paragraphs in the order below.\n"
        "  DO NOT combine, skip, or merge any paragraph. All 5 are mandatory.\n"
        "  Each paragraph MUST open with its bolded system label exactly as shown.\n"
        "  ⚠ CRITICAL PERIOD MATCH RULE: ALL lords and periods cited MUST come from that window's own\n"
        "    entry in evidence_payload.transits.future_timing.windows[].\n"
        ...
        "  PARAGRAPH 1 — open with '**Vimshottari/Parashari:**' then write:\n"
        "    'During the best window ([date range]), the active dasha is [window.mahadasha_lord] Mahadasha "
        "/ [window.antardasha_lord] Antardasha.' Then 1–2 sentences on why these lords support the topic.\n"
        "  PARAGRAPH 2 — open with '**Jaimini/Chara:**' then write EXACTLY:\n"
        "    'In the best window, the Chara Mahadasha activates [window.jaimini_active_sign] "
        "(house [window.jaimini_active_sign_house_from_lagna] from Lagna). "
        "The Chara Antardasha runs in [window.jaimini_active_sub_sign] "
        "(house [window.jaimini_active_sub_sign_house_from_lagna] from Lagna).'\n"
        "    ⚠ BOTH SIGNS MANDATORY: always name jaimini_active_sign (Mahadasha) AND "
        "jaimini_active_sub_sign (Antardasha). Never mention only one.\n"
        "    ⚠ WORDING: use the verb 'activates' — NEVER 'is in', 'is present in', or 'signifies'.\n"
        "    ⚠ HOUSE RULE: derive ALL meaning from jaimini_active_sign_house_from_lagna and "
        "jaimini_active_sub_sign_house_from_lagna — NEVER from the sign name alone.\n"
        "    ⚠ TOPIC MATCH: for each sign, state whether its house directly or only indirectly supports "
        "the question category. Never invent a topic connection from an unrelated house.\n"
        "    Example (property, Jaimini MD house=10, AD house=4): 'In the best window, the Chara Mahadasha "
        "activates Taurus (house 10 from Lagna), governing career and authority — indirect support for "
        "financial stability. The Chara Antardasha runs in Scorpio (house 4 from Lagna), the house of home "
        "and property — a direct property indicator.'\n"
        "  PARAGRAPH 3 — open with '**Yogini:**' then write EXACTLY:\n"
        "    'In the best window, the Yogini major period is [window.yogini_name] "
        "(lord: [window.yogini_alignment.major_lord_name]) and the Yogini sub-period (antardasha) is "
        "[window.sub_yogini_name] (lord: [window.yogini_alignment.sub_lord_name]).'\n"
        "    Then one sentence on how this combination bears on the category.\n"
        "    If sub_yogini_name is null write 'sub-period data unavailable' — do not omit this line.\n"
        "  PARAGRAPH 4 — open with '**Divisional Charts:**' then name the exact chart from "
        "question.primary_divisional_chart (D4/property, D9/marriage, D10/career, etc.). "
        "State what it shows for the antardasha lords of each window.\n"
        "  PARAGRAPH 5 — open with '**Transits & Ashtakavarga:**' then give SAV-based transit quality "
        "for each window's lords. End this paragraph with one sentence explaining why the best window "
        "ranks above the secondary window.\n\n"
        ...
        "    Para 2 — Jaimini/Chara: Name BOTH the Chara Mahadasha sign AND the Chara Antardasha sign. "
        "Write: 'The Chara Mahadasha activates [jaimini_active_sign] (house [jaimini_active_sign_house_from_lagna] "
        "from Lagna). The Chara Antardasha runs in [jaimini_active_sub_sign] "
        "(house [jaimini_active_sub_sign_house_from_lagna] from Lagna).' "
        "Use jaimini_active_sign_house_from_lagna and jaimini_active_sub_sign_house_from_lagna for the houses — "
        "never derive meaning from the sign name alone. "
        "For EACH house, state whether it is a primary house for the question topic. "
        "If a house does NOT match the topic (e.g. house 10 active for a property question), "
        "say so plainly — name that house's domain and call it indirect support. "
        "Do not invent a thematic connection that the house does not carry.\n"
